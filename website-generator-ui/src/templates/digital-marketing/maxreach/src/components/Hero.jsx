      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-20">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <div className="mt-6 space-y-4">
            <h1 className="font-display text-4xl sm:text-5xl md:text-6xl font-semibold flex flex-wrap justify-center items-center gap-2">
              <span className="text-white whitespace-nowrap">
                {d.heading?.prefix || "We build"}
              </span>
              <span className="text-gradient-custom pb-4 min-w-[400px] text-center">
                {currentText}
                <span className="typing-cursor">|</span>
              </span>
            </h1>
            <p className="text-base sm:text-lg text-white/70 leading-relaxed mt-10">
              {d.description || "Default description text goes here."}
            </p>
          </div>
          <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-3">
            <button className="rounded-full bg-gradient-custom px-6 py-3 text-sm leading-6 font-medium text-white shadow-soft transition-all duration-300 hover:scale-105 hover:shadow-lg hover:shadow-pink-500/25 active:scale-95">
              {d.buttons?.getStarted || "Get Started"}
            </button>
            <button className="rounded-full border border-white/20 px-6 py-3 text-sm leading-6 text-white/80 hover:text-white hover:bg-orange-300 hover:border-orange-400 transition-all duration-300 hover:scale-105 active:scale-95">
