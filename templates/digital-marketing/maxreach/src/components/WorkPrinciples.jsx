...
export default function WorkPrinciples({ data }) {
  ...
  const getIcon = (iconName) => {
    ...
    if (IconComponent) {
      return <IconComponent className="text-orange-400 text-lg sm:text-xl" />;
    }
    return <FiEdit3 className="text-orange-400 text-lg sm:text-xl" />; // Fallback
  };
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 sm:gap-10 lg:gap-12 items-center">
          ...
          <div className="space-y-6 sm:space-y-8 order-2 lg:order-1">
            <div className="inline-flex items-center px-3 sm:px-4 py-2 rounded-full border border-orange-400/40 bg-orange-400/10">
            ...
            <div className="flex gap-8 sm:gap-12 lg:gap-16 pt-2">
              {statistics.map((stat, index) => (
                <div key={index}>
                  <div className="text-2xl sm:text-3xl md:text-4xl font-bold text-orange-400 mb-1 sm:mb-2">
                    {stat.value}
                  </div>
                  <div className="text-white/70 text-[10px] sm:text-[11px] md:text-sm font-medium tracking-wider uppercase">
                    {stat.label}
                  </div>
                </div>
              ))}
          ...
          {/* Center Column: Image */}
          <div className="flex justify-center lg:justify-center order-1 lg:order-2 mb-8 lg:mb-0">
            <div className="relative rounded-xl sm:rounded-2xl bg-[#131326] p-2 border border-white/10 shadow-[0_18px_40px_-12px_rgba(0,0,0,0.55)]">
              <div className="overflow-hidden rounded-lg sm:rounded-xl">
                <img
                  src={d?.image?.src || "/people.png"}
                  alt={d?.image?.alt || "Team collaboration"}
                  className="w-[240px] h-[320px] sm:w-[280px] sm:h-[360px] md:w-[320px] md:h-[420px] lg:w-[340px] lg:h-[460px] object-cover filter grayscale hover:grayscale-0 transition duration-500 ease-out"
                  loading="lazy"
                />
              </div>
              <div className="pointer-events-none absolute inset-0 rounded-xl sm:rounded-2xl ring-1 ring-white/10" />
            </div>
          </div>
          ...
          <div className="space-y-6 sm:space-y-8 order-3">
            {features.map((feature, index) => (
              <div key={index} className="flex items-start gap-3 sm:gap-4">
                <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-orange-400/15 border border-orange-400/30 flex items-center justify-center flex-shrink-0">
